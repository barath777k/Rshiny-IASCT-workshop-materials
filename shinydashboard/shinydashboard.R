...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      #*-- Tabs -----------------------------------------------------------------
      menuItem( tags$div("Demographics and", tags$br(), "baseline characteristics", style = "display: inline-block; vertical-align: middle;"), tabName = "demographics", icon = icon("calendar")),
      menuItem("Adverse events", tabName = "adverseEvents", icon = icon("calendar"),
               menuSubItem("Adverse events plot", tabName = "adverseEventsPlot", icon = icon("calendar")),
               menuSubItem("Adverse events data", tabName = "adverseEventsData", icon = icon("calendar"))),
      menuItem("Vital signs", tabName = "vitalSigns", icon = icon("calendar")),
      hr(),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "Demographics bar plot", status = "primary", solidHeader = TRUE,
            collapsible = TRUE,
            plotOutput("plot_demog", height = 250) # UI linked to output$plot_demog generated in the server
          ),
          box(
            title = "Baseline characteristics box plot", status = "primary", solidHeader = TRUE,
            collapsible = TRUE,
            plotOutput("plot_base", height = 250) # UI linked to output$plot_base generated in the server
          )
        ),
        fluidRow(
          box(
